...
impl Tokenizer {
    ...
    /// Train from parquet files directly.
    /// `parquet_paths`: List of paths to parquet files to read from
    /// `vocab_size`: Target vocabulary size
    /// `pattern`: Optional regex pattern for text splitting
    /// `text_column`: Name of the column containing text (default: "text")
    /// `doc_cap`: Maximum characters per document (0 = no limit)
    /// `max_chars`: Maximum total characters to process (0 = no limit)
    #[pyo3(signature = (parquet_paths, vocab_size, pattern=None, text_column="text".to_string(), doc_cap=0, max_chars=0))]
    #[pyo3(text_signature = "(self, parquet_paths, vocab_size, pattern=None, text_column='text', doc_cap=0, max_chars=0)")]
    pub fn train_from_parquet_files(
        &mut self,
        py: pyo3::Python<'_>,
        ...
        log::info!("Reading from {} parquet file(s)", parquet_paths.len());
        
        // Release GIL for the entire parquet reading and processing
        let compiled_pattern = self.compiled_pattern.clone();
        let counts: AHashMap<CompactString, i32> = py.allow_threads(|| {
            let mut total_counts: AHashMap<CompactString, i32> = AHashMap::new();
            let mut total_chars_processed = 0usize;
            let mut total_docs = 0usize;

            for (file_idx, parquet_path) in parquet_paths.iter().enumerate() {
                log::info!("Processing file {}/{}: {}", file_idx + 1, parquet_paths.len(), parquet_path);
                ...
                // Use Arrow reader to read the entire file
                let arrow_reader = parquet::arrow::arrow_reader::ParquetRecordBatchReaderBuilder::try_new(file)
                    .map_err(|e| format!("Failed to create arrow reader: {}", e))
                    .expect("Failed to create arrow reader")
                    .with_batch_size(1024)
                    .build()
                    .map_err(|e| format!("Failed to build arrow reader: {}", e))
                    .expect("Failed to build arrow reader");

                log::info!("  Reading batches from file");
                ...
                for batch_result in arrow_reader {
                    ...
                    let text_col_idx = schema
                        .column_with_name(&text_column)
                        .map(|(idx, _)| idx)
                        .ok_or_else(|| format!("Column '{}' not found in parquet file", text_column))
                    ...
                    let text_array = batch
                        ...
                        .ok_or_else(|| format!("Column '{}' is not a string column", text_column))
                        .expect("Text column is not string type");

                    // Collect documents from this batch
                    let mut batch_docs: Vec<String> = Vec::with_capacity(text_array.len());
                    for i in 0..text_array.len() {
                        if text_array.is_null(i) {
                            continue;
                        }
                        let mut doc_text = text_array.value(i).to_string();
                        
                        // Apply doc_cap if specified
                        if doc_cap > 0 && doc_text.len() > doc_cap {
                            doc_text.truncate(doc_cap);
                        }
                        
                        total_chars_processed += doc_text.len();
                        batch_docs.push(doc_text);
                        
                        // Check max_chars limit
                        if max_chars > 0 && total_chars_processed >= max_chars {
                            log::info!("Reached max_chars limit of {}", max_chars);
                            break;
                        }
                    }

                    total_docs += batch_docs.len();

                    // Process documents in parallel
                    let local_counts: AHashMap<CompactString, i32> = batch_docs
                        .par_iter()
                        .map(|doc| {
                            let mut m: AHashMap<CompactString, i32> = AHashMap::new();
                            for mat in compiled_pattern.find_iter(doc) {
                                let piece = mat.expect("regex match failed").as_str();
                                *m.entry(CompactString::from(piece)).or_default() += 1;
                            }
                            m
                    ...
                    for (k, v) in local_counts {
                        *total_counts.entry(k).or_default() += v;
                    }
                ...
                log::info!("  Processed {} documents, {} chars so far", total_docs, total_chars_processed);

                // Check max_chars limit
                if max_chars > 0 && total_chars_processed >= max_chars {
                    log::info!("Stopping at file {} due to max_chars limit", file_idx + 1);
                    break;
                }
            }

            log::info!("Processed {} documents total, {} unique chunks, {} total chars", 
                      total_docs, total_counts.len(), total_chars_processed);
            total_counts
        });
    ...
    /// Train from a single parquet file.
    /// Convenience wrapper around `train_from_parquet_files` for single file usage.
    /// 
    /// `parquet_path`: Path to a single parquet file to read from
    /// `vocab_size`: Target vocabulary size
    /// `pattern`: Optional regex pattern for text splitting
    /// `text_column`: Name of the column containing text (default: "text")
    /// `doc_cap`: Maximum characters per document (0 = no limit)
    /// `max_chars`: Maximum total characters to process (0 = no limit)
    #[pyo3(signature = (parquet_path, vocab_size, pattern=None, text_column="text".to_string(), doc_cap=0, max_chars=0))]
    #[pyo3(text_signature = "(self, parquet_path, vocab_size, pattern=None, text_column='text', doc_cap=0, max_chars=0)")]
    pub fn train_from_parquet_file(
        &mut self,
        py: pyo3::Python<'_>,
        parquet_path: String,
        vocab_size: u32,
        ...
        // Simply call the multi-file version with a single-element vector
        self.train_from_parquet_files(
            py,
            vec![parquet_path],
            vocab_size,
            pattern,
